...
class _SeqAllToAll(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx: Any, group: dist.ProcessGroup, input: Tensor, scatter_idx: int, gather_idx: int, stream=None, handle=None,type=None,is_fwd=True) -> Tensor:
        ctx.group = group
    ...
    def backward(ctx: Any, *grad_output: Tensor) -> Tuple[None, Tensor, None, None]:
        ...
        return  (None, _SeqAllToAll.apply(ctx.group, *grad_output, ctx.gather_idx, ctx.scatter_idx, ctx.stream,ctx.handle,ctx.type,False), None,None,None,None,None,None)
...
class DistributedAttention(torch.nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, *args: Any) -> Tensor:
        ...
        query_layer = _SeqAllToAll.apply(self.spg, query, self.scatter_idx, self.gather_idx,None,self.overlap_handles,'q') #[1,512,32,32]
        self.dafult_stream.wait_event(key.done_event)
        key_layer = _SeqAllToAll.apply(self.spg, key, self.scatter_idx, self.gather_idx,None, self.overlap_handles,'k') #[1,512,32,32]
        self.dafult_stream.wait_stream(self.sp_stream)
        value_layer= _SeqAllToAll.apply(self.spg, value, self.scatter_idx, self.gather_idx,None,  self.overlap_handles,'v') #[1,512,32,32]
