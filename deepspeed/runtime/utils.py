...
def get_flattened_grad_norm(parameters, norm_type=2, mpu=None, grad_norm_mask=None):
    ...
    Arguments:
        ...
        norm_type (float or int): type of the used p-norm. Can be ``'inf'`` for
            infinity norm.
        grad_norm_mask (Iterable[Tensor] or Tensor): Shape identical to parameters,
            avoid redundant computation of flattened gradient buffer
    ...
    if norm_type == inf:
        ...
        for id, p in enumerate(parameters):
            ...
            if grad_norm_mask is not None:
                param_norm = (p.grad.data * grad_norm_mask[id]).float().norm(norm_type)
            else:
